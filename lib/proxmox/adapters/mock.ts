...
/**
 * Deterministic mock adapter for development and tests.
 * Generates stable data based on node names and time.
 */
...
function genNodeSummary(name: string, now = Date.now()): NodeSummary {
  ...
  const totalMem = 32 * 1024 ** 3; // 32 GiB
  const usedMem = Math.floor(totalMem * (0.3 + r() * 0.6));
  const totalDisk = 500 * 1024 ** 3; // 500 GiB
...
export function createMockClient(): ProxmoxClient {
  async function getClusterSummary(): Promise<ClusterSummary> {
    await new Promise((r) => setTimeout(r, 150)); // latency
    return {
  ...
  async function getNodeMetrics(node: string, rangeSeconds: number): Promise<MetricsSeries> {
    await new Promise((r) => setTimeout(r, 120)); // latency
    ...
    for (const node of NODES) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        vms.push({
          ...
          memory: {
            used: Math.floor(1024 * 1024 * 1024 * (1 + r() * 7)), // 1-8GB used
            max: Math.floor(1024 * 1024 * 1024 * (2 + r() * 14)), // 2-16GB max
          },
          disk: {
            used: Math.floor(1024 * 1024 * 1024 * (10 + r() * 90)), // 10-100GB used
            max: Math.floor(1024 * 1024 * 1024 * (20 + r() * 480)), // 20-500GB max
          },
      ...
      for (let i = 0; i < 2; i++) {
        ...
        vms.push({
          ...
          memory: {
            used: Math.floor(1024 * 1024 * 1024 * (0.5 + r() * 3.5)), // 0.5-4GB used
            max: Math.floor(1024 * 1024 * 1024 * (1 + r() * 7)), // 1-8GB max
          },
          disk: {
            used: Math.floor(1024 * 1024 * 1024 * (2 + r() * 18)), // 2-20GB used
            max: Math.floor(1024 * 1024 * 1024 * (5 + r() * 95)), // 5-100GB max
          },
  ...
  async function performVmAction(node: string, vmid: number, action: VmAction): Promise<{ success: boolean; message?: string }> {
    await new Promise((r) => setTimeout(r, 200 + Math.random() * 800)); // Simulate action time
    
    const r = seededRandom(vmid + action.length);
    const success = r() > 0.05; // 95% success rate
  ...
  async function getSystemLogs(node: string, limit = 50): Promise<LogEntry[]> {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      logs.push({
        timestamp: now - i * 60000 - Math.floor(r() * 300000), // Last few hours
  ...
  async function getBackupJobs(node?: string): Promise<BackupJob[]> {
    ...
    for (const n of targetNodes) {
      for (let i = 0; i < 5; i++) {
        ...
        const duration = Math.floor(r() * 3600000); // Up to 1 hour
        ...
        jobs.push({
          ...
          size: Math.floor(r() * 50 * 1024 ** 3), // Up to 50GB
